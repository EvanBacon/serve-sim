...
export function GridCapacityBanner({ report }: { report: MemoryReport | null }) {
  ...
  return (
    <div className="inline-flex items-center gap-1.5 px-2 py-[3px] rounded-full bg-[#101010] border border-[#222] font-mono text-[11px] text-[#bbb] leading-none">
      <span className="size-1.5 rounded-[3px] shrink-0" style={{ background: dotColor }} />
      <span>{runningSimulators}/{capacity} sims</span>
      <span className="text-[#666]">·</span>
      <span className="text-[#888]">{formatGridBytes(availableBytes)} free</span>
