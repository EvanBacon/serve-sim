...
for arch in "${DUO_ARCHS[@]}"; do
  xcrun swiftc -O -parse-as-library -target "$arch-apple-macos15.0" \
    "$HERE/DuoRenderer.swift" "$HERE/DuoShellMaterials.swift" "$HERE/FastPNG.swift" "$HERE/DuoScreenProjection.swift" "$HERE/DuoHardwareProjection.swift" "$HERE/main.swift" -o "$OUT_DIR/serve-sim-duo-render-$arch"
done
